...
- **enable_bright_region_removal**, **r_erosion**, **r_dilation**, **bright_thresh**: *enable_bright_region_removal* choose whether to remove depth for bright regions. Bright regions are the regions whose pixels have at least one bright core pixels within *r_dilation* pixels in x and y directions. Bright core pixels are pixels which infra1 frame pixel values for all the pixels within the distance of *r_erosion* pixels in both x and y directions are greater than *bright_thresh* value. Pointcloud is not published if *enable_bright_region_removal* is *true* and *infra1* frame is disabled.
...
- **max_angle_surface_to_ray**: remove points consisting surfaces with angles to the optical ray smaller than *max_angle_surface_to_ray*. Disable by giving any negative value (default)
