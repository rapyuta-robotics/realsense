...
- **enable_bright_region_removal**, **r_erosion**, **r_dilation**, **bright_thresh**: *enable_bright_region_removal* choose whether to remove depth for bright regions. Bright regions are the regions whose pixels have at least one bright core pixels within *r_dilation* pixels in x and y directions. Here, a bright core pixel is the center pixel of a (2 * *r_erosion* + 1) * (2 * *r_erosion* + 1) patch, for which all the pixels of *infra1* frame consisting the patch are brighter than *bright_thresh*. Pointcloud is not published if *enable_bright_region_removal* is *true* and *infra1* frame is disabled.
